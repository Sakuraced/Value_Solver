...
def main():
    ...
    for step_num in range(3):
        ...
        for epoch in progress_bar:
            ...
            trajectories, MST = PPO_custom_loss(P=pred_adj, g=Graph, loss_args=loss_args, batch_size=32)
            ...
            for _ in range(epochs):
                one_hot = torch.nn.functional.one_hot(states, num_classes=len(Graph.g.nodes())).to(torch.float)
                probs_list = [pred_adj @ one_hot[i].T for i in range(len(one_hot))]
